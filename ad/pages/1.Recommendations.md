...
# Priority 1: Address Unprofitable Customers

### The Problem

<Value data={unprofitable_impact} column=unprofitable_count/> customers generated <Value data={unprofitable_impact} column=total_losses fmt='$#,##0'/> in losses during Q4 on <Value data={unprofitable_impact} column=revenue_at_risk fmt='$#,##0'/> in revenue. While it seems small, this concentration of small customers represents a high-cost of doing business for the accounts team who need to nurture each of these portfolios.
...
1. **Weeks 1-2**: Issue price increase notifications (10-15%) to all negative-margin customers with 60-day effective date. For customers who have interests in high-margin items or items with high IoH, offer one-time discounts on large orders that can help increase MoQs and liquidate existing stock.
2. **Weeks 3-8**: For customers who reject these strategies, reduce service levels or implement hard-line minimum order quantities
...
**Expected Impact**: Recover 50-70% of losses = <Value data={unprofitable_impact} column=total_losses fmt='$#,##0'/> × 60% = **<Value data={customer_opportunity} column=conservative_recovery fmt='$#,##0'/> to <Value data={customer_opportunity} column=aggressive_recovery fmt='$#,##0'/> annual profit improvement with no incremental spend.**
...
The product portfolio includes a high-number of low-margin, low-volume items that consume resources without delivering profit. While it behooves GWC to remain diversified long-term, having the majority of product types delivering low-margins and low-volume is an ideal place to cut losses and free up investments that can be placed into finding demand for high-margin items. 
...
**Rationalize Product Line with Data**
1. **Immediate**: Reprice "low volume + low margin" products or implement higher MoQs to test demand within current installed customer base
2. **30 days**: Cut products with lowest margins and demand immediately if there is no lift from demand testing
3. **60 days**: Shift sales incentives from revenue to margin dollars to align team with profitability and focus on specific product lines
...
1. **Immediate**: Reduce Q1 2024 production schedules by 15-20% to stop the bleed, focusing on products that do not have seasonal shifts in demand warranting safety stock.
2. **Weeks 1-8**: Launch clearance campaigns for old  inventory: bundle pricing, and volume discounts. Focus on aliging these with new or emerging accounts as an incentive for increasign MoQs.
